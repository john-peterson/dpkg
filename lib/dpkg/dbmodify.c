...
ulist_select(const struct dirent *de)
{
	...
	if (updateslength < 0)
		...
		ohshit(_("updates directory contains files with "
		         "different length names (both %d and %d)"),
		       l, updateslength);
...
modstatdb_can_lock(void)
{
	...
	if (getenv("DPKG_FRONTEND_LOCKED") == NULL) {
		frontendlockfd = open(frontendlockfile,
	...
	case msdbrw_writeifposs:
		...
		if (!db_can_access && errno == ENOENT) {
			if (dir_make_path(dpkg_db_get_dir(), 0755) == 0)
				...
				/* If we cannot create the directory on
				 * read-only modes on read-only filesystems,
				 * make it look like an access error to be
				 * skipped. */
				errno = EACCES;
		}
	...
	for (i = 0; i < nextupdate; i++) {
		...
		if (varbuf_rollback_len(&updatefn_state) > IMPORTANTMAXLEN)
			internerr("modstatdb update entry name '%s' longer than %d",
			          varbuf_rollback_end(&updatefn_state),
			          IMPORTANTMAXLEN);
 ...
 */
void
modstatdb_note(struct pkginfo *pkg)
{
	struct trigaw *ta;
