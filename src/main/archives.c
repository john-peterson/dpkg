                 ...
                 struct pkginfo *pkgbeinginstalled)
{
	...
	if (file->namenode->divert && file->namenode->divert->useinstead) {
		...
		if (divpkgset == pkgtobesaved->set ||
		    divpkgset == pkgbeinginstalled->set) {
			debug_at(dbg_eachfiledetail, "... diverted -- save!");
			return true;
		}
	}

	/* Is the file in the package being installed? If so then it can't
	 * save. */
	...
	while ((thirdpkg = fsys_node_pkgs_iter_next(iter))) {
		...
		/* A Multi-Arch: same package can share files and their
		 * presence in a third package of the same set is not a sign
		...
		/* If !files_list_valid then we have already disappeared this
		 * one, so we should not try to make it take over this shared
		 * directory. */
                      ...
                      struct fsys_namenode *namenode)
{
	...
	for (otherpkg = &pkg->set->pkg;
		...
		/* If we are reinstalling, even if the other package is only
		 * unpacked, we can always make use of the Conffiles hash value
		 * from an initial installation, if that happened at all. */
	...
	if (otherpkg == NULL) {
		...
		debug(dbg_conffdetail,
		      "tarobject found shared conffile, from disk; digest=%s",
		      oldhash);
	}
}
                  ...
                  struct fsys_namenode *namenode)
{
	...
	case TAR_FILETYPE_HARDLINK:
		...
		if (symlink(hardlinkfn.buf, path))
			ohshite(_("error creating sym link '%s'"),
#else
		...
		debug(dbg_eachfiledetail,
		      "tarobject hardlink created, digest=%s",
		      namenode->newhash);
		break;
	...
	case TAR_FILETYPE_DIR:
		...
		break;
	default:
		internerr("unknown tar type '%d', but already checked",
		          te->type);
	}
}
               ...
               struct fsys_namenode *namenode)
{
	if (te->type == TAR_FILETYPE_FILE) {
		...
		namenode->newhash = newhash;
		debug(dbg_eachfiledetail, "tarobject file digest=%s",
		      namenode->newhash);
		...
		linknode = fsys_hash_find_node(te->linkname, FHFF_NONE);
		namenode->newhash = linknode->newhash;
		debug(dbg_eachfiledetail, "tarobject hardlink digest=%s",
		      namenode->newhash);
	}
}
                  ...
                  struct fsys_namenode *namenode)
{
	...
	case TAR_FILETYPE_DIR:
		...
		return;
	case TAR_FILETYPE_SYMLINK:
		/* TODO: Refactor symlink matching. */
                      ...
                      struct varbuf *symlinkfn)
{
	...
	if (ti->linkname[0] == '/') {
		...
		if (lastslash == NULL)
			internerr("tar entry filename '%s' does not contain '/'",
			          fname);
...
tarobject(struct tar_archive *tar, struct tar_entry *ti)
{
	...
	if (nifd->namenode->flags & FNNF_NEW_CONFF) {
		/* If it's a conffile we have to extract it next to the
		 * installed version (i.e. we do the usual link-following). */
		if (conffderef(tc->pkg, &conffderefn, usename))
			usename = conffderefn.buf;
		debug(dbg_conff, "tarobject FNNF_NEW_CONFF deref='%s'",
		      usename);
	}
	...
	if (statr) {
		 ...
		 *
		 * However, it's possible that we were in the middle of some
		 * other backup/restore operation and were rudely interrupted.
		 * So, we see if we have .dpkg-tmp, and if so we restore it.
		 */
		if (rename(fnametmpvb.buf, fnamevb.buf)) {
			/* Trying to remove a directory or a file on a
			 * read-only filesystem, even if non-existent,
			 * always returns EROFS. */
			if (errno == EROFS) {
				/* If the file does not exist the access()
				 * function will remap the EROFS into an ENOENT,
				 * otherwise restore EROFS to fail with that. */
			...
			if (errno != ENOENT && errno != ENOTDIR)
				ohshite(_("unable to clean up mess surrounding '%s' "
				        ...
				        ti->name);
			debug(dbg_eachfiledetail,
			      "tarobject %s nonexistent as %s",
			      ti->name, fnamevb.buf);
		} else {
			debug(dbg_eachfiledetail,
			      "tarobject %s restored tmp to main %s",
			      ti->name, fnamevb.buf);
			statr = lstat(fnamevb.buf, &stab);
			if (statr)
	...
	case TAR_FILETYPE_SYMLINK:
		...
		if (!statr && S_ISDIR(stab.st_mode)) {
			debug(dbg_eachfiledetail,
			      "tarobject %s symlink exists as directory %s",
			      ti->name, fnamevb.buf);
			existingdir = true;
	...
	case TAR_FILETYPE_DIR:
		...
		if (!stat(fnamevb.buf, &stabtmp) && S_ISDIR(stabtmp.st_mode)) {
			debug(dbg_eachfiledetail,
			      "tarobject %s directory exists as %s",
			      ti->name, fnamevb.buf);
			existingdir = true;
		}
		break;
	...
	if (!existingdir) {
		...
		while ((otherpkg = fsys_node_pkgs_iter_next(iter))) {
			...
			/* A pkgset can share files between its instances.
			 * Overwriting is allowed when they are not getting
			 * in sync, otherwise the file content must match
			 * the installed file. */
			...
			if (nifd->namenode->divert &&
				...
				if (otherpkg->set == divpkgset ||
				    tc->pkg->set == divpkgset)
					continue;
			}

			/* If the new object is a directory and the previous
			 * object does not exist assume it's also a directory
			 * and skip further checks.
			 * XXX: Ideally with more information about the
			 * installed files we could perform more clever
			 * checks. */
			...
			/* Is the package with the conflicting file in the
			 * “config files only” state? If so it must be a
			 * config file and we can silently take it over. */
			...
			if ((nifd->namenode->flags & FNNF_NEW_CONFF) &&
				...
				if (conff) {
					...
					/* process_archive() will have copied
					 * its hash already. */
					continue;
				}
			}
	...
	if (refcounting) {
		debug(dbg_eachfiledetail,
		      ...
		      fnamevb.buf);
		if (nifd->namenode->flags & FNNF_NEW_CONFF) {
			md5hash_prev_conffile(tc->pkg, oldhash, fnamenewvb.buf,
			                      nifd->namenode);
	...
	if (refcounting && !in_force(FORCE_OVERWRITE)) {
		/* If we are not forced to overwrite the path and are
		 * refcounting, just compute the hash w/o extracting
		 * the object. */
		tarobject_hash(tc, ti, nifd->namenode);
	} else {
		/* Now, at this stage we want to make sure neither of
		 * .dpkg-new and .dpkg-tmp are hanging around. */
		path_remove_tree(fnamenewvb.buf);
		path_remove_tree(fnametmpvb.buf);

		/* Now we start to do things that we need to be able to undo
		 * if something goes wrong. Watch out for the CLEANUP comments
		 * to keep an eye on what's installed on the disk at each
		 * point. */
		push_cleanup(cu_installnew, ~ehflag_normaltidy,
		             1, nifd->namenode);

		/*
		 * CLEANUP: Now we either have the old file on the disk, or
		 * not, in its original filename.
		 */

		/* Extract whatever it is as .dpkg-new ... */
		tarobject_extract(tc, ti, fnamenewvb.buf, &nodestat,
		                  nifd->namenode);
	}
	...
	if (statr) {
		...
		if (ti->type == TAR_FILETYPE_DIR || S_ISDIR(stab.st_mode)) {
			/* One of the two is a directory - can't do atomic
			 * install. */
			debug(dbg_eachfiledetail,
			...
			/* We can't make a symlink with two hardlinks, so
			 * we'll have to copy it. (Pretend that making a copy
                    ...
                    struct pkginfo *pkg_install, const char *why)
{
	if (force_breaks(pdep)) {
		warning(_("ignoring dependency problem with installation of %s:\n%s"),
		        pkgbin_name(pkg_install, &pkg->available, pnaw_nonambig),
		        why);
		return 2;
		...
		notice(_("no, cannot proceed with installation of %s "
               ...
               struct pkginfo *pkg_removal, const char *why)
{
	...
	if (force_depends(pdep)) {
		...
		notice(_("no, cannot proceed with removal of %s "
		         "(--auto-deconfigure will help):\n%s"),
		       pkg_name(pkg_removal, pnaw_nonambig), why);
		return 0;
	}
...
void
check_breaks(struct dependency *dep, struct pkginfo *pkg, const char *pfilename)
{
	...
	if (fixbydeconf && f_autodeconf) {
		...
		if (fixbydeconf->clientdata->istobe != PKG_ISTOBE_NORMAL)
			internerr("package %s being fixed by deconf is not to be normal, "
			          ...
			          fixbydeconf->clientdata->istobe);

		notice(_("considering deconfiguration of %s, "
		         "which would be broken by installation of %s ..."),
		       pkg_name(fixbydeconf, pnaw_nonambig),
		       pkgbin_name(pkg, &pkg->available, pnaw_nonambig));

		ok = try_deconfigure_can(fixbydeconf, dep->list, pkg,
		                         varbuf_str(&why));
		if (ok == 1) {
               ...
               const char *pfilename)
{
	...
	struct varbuf conflictwhy = VARBUF_INIT;
	struct varbuf removalwhy = VARBUF_INIT;
	...
	if (fixbyrm) {
		...
		if (((pkg->available.essential ||
		      pkg->available.is_protected) &&
			...
			if (!(fixbyrm->status == PKG_STAT_INSTALLED ||
				...
				if (!pdep) {
					/* If we haven't found a reason not to
					 * yet, let's look some more. */
			...
			if (!pdep) {
				/* This conflict is OK - we'll remove the
				 * conflictor. */
...
archivefiles(const char *const *argv)
{
	...
	if (f_recursive) {
		...
		if (!*argv)
			badusage(_("--%s --recursive needs at least one path argument"),
			         cipaction->olong);
		...
		for (ap = argv; *ap; ap++) {
			...
			tree = treewalk_open((const char *)*ap,
			                     TREEWALK_FOLLOW_LINKS, NULL);
	...
	for (i = 0; argp[i]; i++) {
		...
		if (!S_ISREG(st.st_mode))
			ohshit(_("archive '%s' is not a regular file"),
			       argp[i]);
	}
...
wanttoinstall(struct pkginfo *pkg)
{
	...
	rc = dpkg_version_compare(&pkg->available.version,
